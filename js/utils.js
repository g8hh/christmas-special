...
function respecBuyables(layer) {
	...
	if (!player[layer].noRespecConfirm && !tmp[layer].buyables.noRespecConfirm && !confirm(tmp[layer].buyables.respecMessage || "Are you sure you want to respec? This will force you to do a \"" + (tmp[layer].name ? tmp[layer].name : layer) + "\" reset as well!")) return
...
function layOver(obj1, obj2) {
	for (let x in obj2) {
		if (obj2[x] instanceof Decimal) obj1[x] = new Decimal(obj2[x])
			...
			if (!(obj1[x] instanceof Object)) obj1[x] = {};
